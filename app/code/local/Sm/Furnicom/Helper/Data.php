...
class Sm_Furnicom_Helper_Data extends Mage_Core_Helper_Abstract{
	...
	function get($attributes=array()){
		$data           = $this->defaults;
		$general        = Mage::getStoreConfig("furnicom_cfg/general");
		$detail_furnicom = Mage::getStoreConfig("furnicom_cfg/detail_furnicom");
		$rich_snippets_setting = Mage::getStoreConfig("furnicom_cfg/rich_snippets_setting");
		$social_furnicom = Mage::getStoreConfig("furnicom_cfg/social_furnicom");
		$advanced 	    = Mage::getStoreConfig("furnicom_cfg/advanced");
		if (!is_array($attributes)) {
			$attributes = array($attributes);
		}
		if (is_array($general))	
		$data = array_merge($data, $general);
		if (is_array($detail_furnicom))
		$data = array_merge($data, $detail_furnicom);
		if (is_array($rich_snippets_setting)) 				
		$data = array_merge($data, $rich_snippets_setting);
		if (is_array($social_furnicom))
		$data = array_merge($data, $social_furnicom);
	...
	{
		/*$design_config=Mage::getStoreConfig('furnicom_cfg/general/theme_color');
    ...
    {
        $lightBoxTypeJs = Mage::helper('furnicom/config')->getProductSetting('lightbox_types');
    ...
    {
        $headerStyle = Mage::helper('furnicom/config')->getThemeLayout('header_style');
    ...
    {
        $footerStyle = Mage::helper('furnicom/config')->getThemeLayout('footer_style');
	...
	/* public function getDetailTemplate($store = null)
    {
		if (isset($_REQUEST['detail_style'])) {
			...
			$detail_style = Mage::helper('furnicom/config')->getProductSetting('detail_style');
		}	
